...
export async function run() {
  ...
  $('#main-col-body h3').each((_i, heading) => {
      ...
      if (el.is('div.itemizedlist') || el.is('ul')) {
        ...
        list.find('> li').each((_j, li) => {
          ...
          if (text.includes('Data type')) {
            const match = text.match(/Data type\s*–\s*(\w+)/);
            if (match) dataType = match[1];
          }
          if (text.includes('Value type')) {
            multivalued = text.toLowerCase().includes('multivalued');
          }
    ...
    const types = dataType ? [multivalued ? `ArrayOf${dataType}` : dataType] : [];
    conditionKeys.push({ name: normalizeKeyName(name), types, availability, description });
  });
